...
#define kCustomKeyboardHeight kScreen.height*0.3
...
{
    if (!_voiceView) {
        _voiceView = [[UIButton alloc] initWithFrame:CGRectMake(CGRectGetMaxX(self.voiceButton.frame) + 5, 7, kScreen.width - 115, kInputHeight)];
        ...
        _voiceView.backgroundColor = HM_COLOR(243, 243, 243);
        _voiceView.layer.borderColor = HM_COLOR(210, 210, 210).CGColor;
...
-(void)initView
{
    ...
    self.backgroundColor = HM_COLOR(243, 243, 243);
...
-(void)setupSubviews
{
    ...
    line.backgroundColor = HM_COLOR(220, 220, 220);
    ...
    self.textInput = [[UITextView alloc] initWithFrame:CGRectMake(CGRectGetMaxX(self.voiceButton.frame) + 5, (self.height - kInputHeight)/2, kScreen.width - 115, 37)];
    self.textInput.font = [UIFont systemFontOfSize:17];
    self.textInput.layer.cornerRadius = 3;
    self.textInput.layer.borderColor = HM_COLOR(210, 210, 210).CGColor;
...
- (void)setTextPlaceHold:(NSString *)textPlaceHold{
    _textPlaceHold = textPlaceHold;
    self.textInput.text = textPlaceHold;
    self.textInput.textColor = [UIColor grayColor];
}

#pragma mark UITextViewDelegate

- (void)textViewDidBeginEditing:(UITextView *)textView{
    if ([_textPlaceHold containsString:textView.text]) {
        self.textInput.text = @"";
...
- (void)textViewDidChange:(UITextView *)textView
{
    ...
    if (self.textInput.scrollEnabled) {
        ...
        self.y = kScreen.height - _keyboardHeight - _textInputMaxHeight - 5 - 8;
        ...
        self.y = kScreen.height - _keyboardHeight - _textInputHeight - 5 - 8;
...
- (void)voiceButtonTouchDown
{
    ...
    hud.customView.backgroundColor = [UIColor colorWithRed:116/255 green:116/255 blue:116/255 alpha:0.66];
    hud.customView.layer.cornerRadius = 8;
...
{
    hud.labelColor = HM_COLOR(158, 56, 54);
...
{
    hud.labelColor = [UIColor clearColor];
    hud.labelText = @"手指上滑,取消发送";
