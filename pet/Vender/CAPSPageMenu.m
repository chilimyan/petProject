...
NSString * const CAPSPageMenuOptionTextAndLineMargin                       = @"menuOrignY";
...
- (instancetype)initWithViewControllers:(NSArray *)viewControllers frame:(CGRect)frame options:(NSDictionary *)options
{
    ...
    if (options) {
        for (NSString *key in options) {
            if ([key isEqualToString:CAPSPageMenuOptionTextAndLineMargin]) {
                _menuOrignY = [options[key] floatValue];
            }else if ([key isEqualToString:CAPSPageMenuOptionSelectionIndicatorHeight]) {
...
- (void)configureUserInterface
{
    ...
    for (UIViewController *controller in _controllerArray) {
        ...
        if (_useMenuLikeSegmentedControl) {
            menuItemFrame = CGRectMake(self.view.frame.size.width / (CGFloat)_controllerArray.count * (CGFloat)index, _menuOrignY, (CGFloat)self.view.frame.size.width / (CGFloat)_controllerArray.count, _menuHeight);
            ...
            menuItemFrame = CGRectMake(_totalMenuItemWidthIfDifferentWidths + _menuMargin + (_menuMargin * index), _menuOrignY, _menuItemWidth, _menuHeight);
            ...
            if (_centerMenuItems && index == 0.0) {
                ...
                menuItemFrame = CGRectMake(_startingMenuMargin + _menuMargin, _menuOrignY, _menuItemWidth, _menuHeight);
            } else {
                menuItemFrame = CGRectMake(_menuItemWidth * index + _menuMargin * (index + 1) + _startingMenuMargin, _menuOrignY, _menuItemWidth, _menuHeight);
            }
        }
