...
#define DEVICE_WIDTH_LAYER [UIScreen mainScreen].bounds.size.width

@interface CLProgressLayer ()
...
@implementation CLProgressLayer
...
- (void)initPath{
    ...
    [path addLineToPoint:CGPointMake(DEVICE_WIDTH_LAYER, 2)];
    self.path = path.CGPath;
    self.strokeEnd = 0;
}

- (void)setProgressStyle:(CLProgressStyle)progressStyle {
    _progressStyle = progressStyle;
    if (progressStyle == CLProgressStyleGradual) {
...
- (void)setProgressColor:(UIColor *)progressColor {
    ...
    if (_progressStyle == CLProgressStyleNoraml) {
        self.strokeColor = progressColor.CGColor;
    } else {
        self.progressStyle = CLProgressStyleGradual;
    }
}
...
- (void)progressChanged:(NSTimer *)timer {
    ...
    if (_progressStyle == CLProgressStyleGradual) {
        _gradientLayer.locations = @[@(self.strokeEnd/2), @(self.strokeEnd)];
        _gradientLayer.frame = CGRectMake(0, 0, DEVICE_WIDTH_LAYER*self.strokeEnd, 2);
    }
}
